...
class ExtensoPtTest < Minitest::Test
  ...
  def test_basico
    ...
    assert_equal 'ZERO REAIS', 0.extenso(lc: :br, mplural: 'REAIS')
  ...
  def test_useful2
    # por defeito moeda do brazil é configurada somente se nao for indicado moeda singular/plural
    assert_equal 'QUATORZE DÓLARES E UM CÊNTIMO', 14.01.extenso(lc: :br, mplural: 'DÓLARES')
    assert_equal 'QUATORZE EUROS E UM CÊNTIMO',   14.01.extenso(lc: :br, msingular: 'EURO')
    assert_equal 'QUATORZE REAIS E UM CENTAVO',   14.01.extenso(lc: :br)
    assert_equal 'QUATORZE REAIS E DEZ CENTAVOS', 14.10.extenso(lc: :br, fsingular: 'CENT')
    assert_equal 'QUATORZE REAIS E UM CENTAVO',   14.01.extenso(lc: :br, fplural: 'CENTS')
  end
