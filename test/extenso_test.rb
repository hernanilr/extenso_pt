...
class ExtensoPtTest < Minitest::Test
  ...
  def test_use1
    assert_equal 'UM DÓLAR E UM CÊNTIMO',
                 1.010.extenso(moeda_singular: 'DÓLAR')
    assert_equal 'DEZ DÓLARES E DEZ CÊNTIMOS',
                 10.10.extenso(moeda_plural: 'DÓLARES')
    # por defeito <plural> = <singular> mais 'S'
    assert_equal 'CATORZE REAIS E DEZ CENTAVOS',
                 14.10.extenso(moeda_plural: 'REAIS',
                               fracao_singular: 'CENTAVO')
  ...
  def test_use2
    ...
    assert_equal 'DEZ EUROS E UM CENTAVO',
                 10.01.extenso(fracao_plural: 'CENTAVOS')
    # por defeito <plural> = <singular> mais 'S'
    assert_equal 'DEZ EUROS E DEZ CENTAVOS',
                 10.10.extenso(fracao_singular: 'CENTAVO')
  end

  def test_use3
    # moeda do brazil configurada se nao for indicado moeda singular/plural
    assert_equal 'QUATORZE DÓLARES E UM CÊNTIMO',
                 14.01.extenso(lc: :br, moeda_plural: 'DÓLARES')
    assert_equal 'QUATORZE EUROS E UM CÊNTIMO',
                 14.01.extenso(lc: :br, moeda_singular: 'EURO')
    assert_equal 'QUATORZE REAIS E UM CENTAVO', 14.01.extenso(lc: :br)
    assert_equal 'QUATORZE REAIS E DEZ CENTAVOS',
                 14.10.extenso(lc: :br, fracao_singular: 'CENT')
    assert_equal 'QUATORZE REAIS E UM CENTAVO',
                 14.01.extenso(lc: :br, fracao_plural: 'CENTS')
  end
  ...
  def test_use4
    ...
    assert_equal 'CEM MIL TRILIÕES DE REAIS',
                 (10**23).extenso(moeda_plural: 'REAIS') # escala longa
  ...
  def test_use5
    ...
    assert_equal ({ a: ['TRÊS EUROS', 'DEZ EUROS'], b: 'DOIS EUROS' }),
                 { a: [3, 10], b: 2 }.extenso
  end
  ...
  def test_unidades
    assert_equal ['TRÊS EUROS', 'QUATRO EUROS', 'CINCO EUROS', 'SEIS EUROS',
                  ...
                  'DEZOITO EUROS', 'DEZANOVE EUROS', 'VINTE EUROS'],
                 (3..20).extenso
  end
end
